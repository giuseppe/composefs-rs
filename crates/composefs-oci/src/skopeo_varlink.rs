...
///
/// Returns a `zlink::tokio::unix::Connection` and a guard that keeps skopeo alive.
pub(crate) fn open_skopeo_varlink(
    image_ref: &str,
) -> Result<(zlink::tokio::unix::Connection, SkopeoGuard)> {
    ...
    let (_img_val, _) = proxy_call(&sock, "OpenImage", vec![cs_ref.clone().into()])?;
    ...
    tracing::debug!("got org.composefs.Oci varlink socket from skopeo for {cs_ref}");
...
pub(crate) async fn import_via_skopeo_proxy<ObjectID: FsVerityHashValue>(
    ...
    let (mut conn, _guard) =
        tokio::task::spawn_blocking(move || open_skopeo_varlink(&image_ref_owned))
            .await
            .context("spawn_blocking(open_skopeo_varlink) failed")??;

    // GetImage returns manifest, config, and storage layer IDs.
    tracing::debug!("Oci.GetImage -> skopeo for {image_ref}");
    ...
    tracing::debug!(
        "Oci.GetImage <- manifest {} bytes, config {} bytes, {} storage layers",
        image_reply.manifest.len(),
        image_reply.config.len(),
        image_reply.layer_digests.len()
    );
